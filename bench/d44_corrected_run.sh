#!/usr/bin/env bash
# D44 CORRECTED FACTORIAL: the whole lane re-measured on the fixed harness.
#
# bench/d44_harness_counter.txt established that the old harness's shared per-fetch counter was a
# CEILING and only a ceiling: BASE's slope did not move (x0.144 -> x0.147) while the
# no-policy-update arm went x0.740 -> x2.630 and 43.7 -> 152.1 M/s at 16 threads. So the cell
# ORDERING of every earlier run stands, and every CEILING figure -- and D44's x0.5 bar, which was
# set against one -- has to be re-taken. This is that re-take.
#
# ⛔ WHAT IS DELIBERATELY NOT PRE-REGISTERED HERE: a pass/fail bar. The old one (x0.5, against a
# x0.579 ceiling) was calibrated against a capped instrument and restating it is the lead's call,
# not this script's. What this file produces is the four cells and the headroom fraction; the
# criterion goes on top of it afterwards, which is the honest order.
#
...
OUT=${OUT:-bench/d44_corrected.txt}
BASE_BIN=${BASE_BIN:?}; C1_BIN=${C1_BIN:?}; PAIR_BIN=${PAIR_BIN:?}; STUB_BIN=${STUB_BIN:?}
SUITE_LOCK=${SUITE_LOCK:-/tmp/ferrodb-suite.lock}

for b in "$BASE_BIN" "$C1_BIN" "$PAIR_BIN" "$STUB_BIN"; do
...

{
  ...
  echo "#"
  echo "# ALL FOUR ARMS ON THE FIXED HARNESS (ec0c53c): per-thread counts, nothing shared in"
  echo "# the timed loop. Every figure in this file supersedes its old-harness counterpart."
  echo "#"
  echo "# BASE = c56127f                          (no mirror, eager touch)"
  echo "# C1   = 8bf34ad                          (mirror,    eager touch)"
  echo "# PAIR = $(git rev-parse --short HEAD) TOUCH_BATCH=512  (mirror, BATCHED touch)  <- the candidate"
  echo "# STUB = mirror + NO policy update        (unshippable CEILING)     SCAFFOLD"
  echo "#"
