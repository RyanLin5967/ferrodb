...
* `bench/evidence/F11-mutants.md` — 11 mutants, raw cargo output in `bench/evidence/raw/`.
* `bench/evidence/F11-suite.txt` — per-target suite under CI's exact `RUSTFLAGS`. **1715 pass, 0 fail.**
* `bench/evidence/F11-reruns.txt` — the four targets the loaded batch could not finish, 3/3 each.
* `bench/evidence/F11-final.txt` — confirmation run after the last fix, with its scope named.
