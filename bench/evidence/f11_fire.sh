...
# gate is not optional. Raw cargo output is written to bench/evidence/raw/ FIRST and the summary is
...
OUT=bench/evidence/F11-mutants.md
RAW=bench/evidence/raw
mkdir -p $RAW
NAME=$1
LIBF=${2:-}
INTF=${3:-}
...
python3 bench/evidence/f11_mutate.py "$NAME" >/dev/null || { echo "$NAME: could not apply"; exit 1; }
printf '\n### %s\n' "$NAME" >> $OUT
