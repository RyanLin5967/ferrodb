...
M = [
    ...
    ("M28", SNAP, "        self.note_config_in_log(cur.meta.config.clone(), out)?;",
     ...
     "a_snapshot_whose_configuration_is_damage_is_refused_before_anything_is_spent", "lib"),
    ...
    ("M36", NODE, "        let through = self.applied.saturating_sub(keep);",
     ...
     "a_follower_partitioned_past_log_retention_rejoins_by_snapshot_and_converges",
     "integration_cluster_snapshot"),
    ("M37", NODE, "        if self.spooled != meta.total_bytes || spooled_len != meta.total_bytes {",
     "        if false {",
     "consensus::node::tests_node::a_spool_short_of_what_was_digested_is_refused_rather_than_installed", "lib"),
    # The other half: a driver that placed every chunk must still install. Without this, M37's kill
    # would be satisfied by a driver that refused every install.
    ("M42", NODE, "        let starting = offset == 0;\n        let continuing = self.spooled_header == Some(header) && self.spooled == offset;\n        if !starting && !continuing {\n            return Ok(());\n        }",
     "        return Ok(());",
     "consensus::node::tests_node::a_spool_that_kept_up_installs_rather_than_being_refused", "lib"),

    # ---- the rules the fresh-context review added --------------------------------------------------
    ("M43", SNAP, "        if meta.config.at() == self.cfg.at() && meta.config != self.cfg {",
     "        if false {",
     "a_snapshot_whose_configuration_is_damage_is_refused_before_anything_is_spent", "lib"),
    ("M44", SNAP, "        if meta.validate().is_err() {\n            self.refuse_snapshot(from, out);\n            return;\n        }\n        // The configuration is checked here",
     "        // moved below the office change, which is the defect\n        // The configuration is checked here",
     "a_refused_snapshot_does_not_change_this_nodes_office", "lib"),
    ("M45", SNAP, "            if len > Self::MAX_SIDECAR_BYTES {",
     "            if false {",
     "a_header_claiming_an_unbounded_sidecar_section_is_refused", "lib"),
    ("M46", SNAP, "        if !redo.is_empty() {",
     "        if false {",
     "the_redo_window_that_travels_with_a_snapshot_is_replayed_into_it",
     "integration_cluster_snapshot"),
    ("M47", SNAP, "        self.wal.truncate(0)?;",
     "        // mutant: the receiver keeps a log describing the database this install replaced",
     "an_install_discards_the_receivers_own_wal",
     "integration_cluster_snapshot"),
    ("M48", SNAP, "        if install_was_interrupted(&self.paths.page_file) {",
     "        if false {",
     "a_node_left_part_way_through_an_install_refuses_to_start",
     "integration_cluster_snapshot"),
    ("M49", SNAP, "        std::fs::remove_file(&marker)",
     "        std::fs::metadata(&marker).map(|_| ())",
     "a_completed_install_leaves_no_marker_behind",
     "integration_cluster_snapshot"),
    ("M50", NODE, "        if let Some(store) = opts.snapshots.as_ref() {\n            store.check_ready()?;\n        }",
     "        // mutant: a half-replaced node joins the cluster anyway",
     "a_node_left_part_way_through_an_install_refuses_to_start",
     "integration_cluster_snapshot"),
