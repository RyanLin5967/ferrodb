...
fn example_bin(name: &str) -> PathBuf {
    ...
    let newest = [walk_newest(Path::new("src")), own_src].into_iter().flatten().max();
    if let Some(src) = newest {
        assert!(
            bin_time >= src,
            "{} is older than src/ or examples/; run: cargo build --examples",
            bin.display()
        );
    }
    bin
...
fn walk_newest(dir: &Path) -> Option<std::time::SystemTime> {
    ...
    for e in std::fs::read_dir(dir).ok()?.flatten() {
        ...
        // Enforced by `test_only_sources_are_cfg_test_gated`, not assumed.
